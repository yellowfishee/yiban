...
const THEMES: {
	id: ThemeMode;
	name: string;
	colors: { primary: string; text: string; bg: string };
}[] = [
	{
		id: "xuanqing",
		name: "玄青",
		colors: { primary: "#C73E3A", text: "#1A2B3C", bg: "#F5F0E8" },
	},
	{
		id: "dailan",
		name: "黛蓝",
		colors: { primary: "#D4A5A5", text: "#3D5A73", bg: "#FAF8F5" },
	},
	{
		id: "mojin",
		name: "墨金",
		colors: { primary: "#C9A84C", text: "#1C1C1C", bg: "#EDE8DC" },
	},
];
...
export default function SettingsPage() {
	...
	const handleSaveProfile = async () => {
		try {
			setSaving(true);
			const finalNickname =
				editNickname.trim() || "易友" + Math.floor(Math.random() * 10000);
			let finalAvatar = editAvatar;

			// 如果是临时文件，需要先上传
			if (
				editAvatar.startsWith("http://tmp") ||
				editAvatar.startsWith("wxfile://")
			) {
				try {
		...
		<View className="settings-page">
					...
					<View className="settings-page__user">
						...
						<View className="settings-page__user-info">
							<Skeleton
								width="180px"
								height="36px"
								className="settings-page__skeleton-name"
							/>
							<Skeleton
								width="120px"
								height="24px"
								className="settings-page__skeleton-id"
							/>
						</View>
					</View>
				</View>
				...
				user && (
					<View className="settings-page__card">
						<View className="settings-page__user" onClick={handleEditProfile}>
							{user.avatar ? (
								<Image
									className="settings-page__user-avatar-img"
									src={getFullAvatarUrl(user.avatar)}
									mode="aspectFill"
								/>
							) : (
								<View
									className="settings-page__user-avatar"
									style={{ background: currentTheme.colors.primary }}
								>
									<Text className="settings-page__user-avatar-text">
										{user.nickname?.charAt(0) || "易"}
									</Text>
								</View>
							)}
							<View className="settings-page__user-info">
								<Text className="settings-page__user-name">
									{user.nickname || "易伴用户"}
								</Text>
								<Text className="settings-page__user-id">
						...
						{stats && (
							<View className="settings-page__stats">
								<View className="settings-page__stat">
									<Text className="settings-page__stat-value">
										{stats.totalCheckins}
									</Text>
					...
					{THEMES.map((t) => {
						...
						return (
							...
							>
								...
								>
									<View
										className="settings-page__theme-preview-header"
										style={{ background: t.colors.primary }}
									/>
									<View className="settings-page__theme-preview-body">
										<View
											className="settings-page__theme-preview-text"
											style={{ background: t.colors.text }}
										/>
										<View
											className="settings-page__theme-preview-text settings-page__theme-preview-text--short"
											style={{ background: t.colors.text }}
										/>
									</View>
								</View>
								<Text
									className={`settings-page__theme-name ${isSelected ? "settings-page__theme-name--active" : ""}`}
								>
									{t.name}
								</Text>
								{isSelected && (
									<View className="settings-page__theme-check">✓</View>
								)}
							</View>
						);
					})}
			...
			<View className="settings-page__card">
				...
				<View className="settings-page__item">
					...
					>
						<View
							className={`settings-page__toggle-knob ${simplified ? "settings-page__toggle-knob--on" : ""}`}
						/>
					</View>
				</View>
					...
					<View className="settings-page__font-sizes">
						{FONT_SIZES.map((fs) => {
							...
							return (
								...
								>
									<Text className="settings-page__font-size-text">
										{fs.name}
									</Text>
								</View>
							);
			...
			>
				<View className="settings-page__edit-modal">
					...
					>
						<Image
							className="settings-page__edit-avatar"
							src={getFullAvatarUrl(editAvatar)}
							mode="aspectFill"
						/>
						<Text className="settings-page__edit-avatar-hint">
							点击更换头像
						</Text>
					</Button>
