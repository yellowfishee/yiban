...
function cleanExtractedText(text: string): string {
	...
	c = c.replace(
		/^[*\s]*(尝试\s*\d*[（(].*?[)）][：:]?\s*|草稿\d*[（(].*?[)）]?[：:]\s*|起草内容[：:]\s*|润色后?[：:]\s*|最终回答[：:]\s*|最终版本[：:]\s*|回答[：:]\s*|优化[版本]*[：:]\s*|修改后[：:]\s*)/,
		"",
	);
...
function extractFinalAnswer(reasoning: string): string {
	...
	//   "尝试 2（优化基调和词汇）：*吾乃..."
	const draftPattern =
		/(?:草稿|尝试)\s*\d*[（(].*?[)）]?[：:]\s*([\s\S]+?)(?=(?:草稿|尝试)\s*\d*[（(]|字数检查|对照约束|$)/g;
	const drafts: string[] = [];
	let draftMatch;
	...
	const simpleDraftPattern =
		/草稿\d+[：:]\s*([\s\S]+?)(?=草稿\d+[：:]|字数检查|对照约束|$)/g;
	...
	const candidates = segments
		.map((seg, idx) => {
			...
			if (
				/^(步骤|分析|构思|首先|其次|然后|综上|总结|以下|根据|点评)/.test(text)
			)
				score -= 4;
...
async function callGLMAPI(
	...
	if (message?.reasoning_content) {
		console.log(
			"[Agent] reasoning_content length:",
			message.reasoning_content.length,
		);
		console.log(
			"[Agent] reasoning_content last 200:",
			message.reasoning_content.slice(-200),
		);
