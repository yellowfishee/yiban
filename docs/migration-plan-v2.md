...
- [x] 微信分享功能（ShareButton + useShareAppMessage）
- [x] 默认头像占位符替换（SVG data URI）
- [x] 短信服务改进（错误计数 + SMS provider 支持）
...
3. **达到 500 UV 后**：配置微信广告单元 ID（`your_ad_unit_id` → 真实 ID）
